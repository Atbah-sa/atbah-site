...
export default async function handler(req, res) {
  ...
  const qs = new URL(req.url, 'http://x').searchParams; const Q = (k) => (req.query && req.query[k] != null ? req.query[k] : qs.get(k));
  const token = process.env.LEADS_READ_TOKEN;
  if (!token || String(Q('token') || '') !== token) return res.status(401).json({ error: 'unauthorized' });
  ...
  const hours = Math.min(24 * 30, Math.max(1, +Q('hours') || 48));
  const srcFilter = String(Q('source') || '');
  try {
